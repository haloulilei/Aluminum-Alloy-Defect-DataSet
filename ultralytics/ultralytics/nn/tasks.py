...
from ultralytics.nn.model import MAE_Encoder
from ultralytics.nn.model import MAE_Decoder
...
from ultralytics.nn.LSKA import C2f_LSKA_Attention, LSKA, LSKA_Attention
from ultralytics.nn.EMA_attention import EMA_attention
from ultralytics.nn.ODConv import ODConv
from ultralytics.nn.LSKA import LSKA
...
def parse_model(d, ch, verbose=True, no=None, gw=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, GGhostRegNet, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, RepC3, SEAttention,ContextAggregation, BoTNet, CBAM,LightConv,RepConv, SpatialAttention,Involution, CARAFE, VoVGSCSP, VoVGSCSPC,GSConv,HorBlock, SwinTransformer, MobileOneBlock,MobileViT,MobileViTBlock,MV2Block,CondConv2D,MBConv,FusedMBConv,stem,RepViTblock,DSConv,DySnakeConv,C2f_DySnakeConv,Bottleneck_DySnakeConv,C2f_LSKA_Attention,EMA_attention,ODConv):
            ...
            args = [ch[f], *args]
        elif m in {MAE_Encoder}:
            args = [ch[f], *args]
        elif m in {MAE_Decoder}:
            args = [ch[f], *args]
        elif m is AIFI:
            args = [ch[f], *args]
